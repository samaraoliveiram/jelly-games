defmodule JellyWeb.DataEntryComponents do
    ...
    <.field_wrapper id={@id} name={@name} label={@label} errors={@errors}>
      <input
        ...
        class={[
          ...
          "text-gray-900 border-gray-300 focus:outline-purple-500 focus:outline-2 text-sm font-light leading-6",
  ...
  defp field_wrapper(assigns) do
    ~H"""
    <div class={["relative", @errors != [] && "show-errors"]}>
      {render_slot(@inner_block)}
      <.label for={@id}>{@label}</.label>
  ...
  def simple_form(assigns) do
    ~H"""
    <.form :let={f} class="flex flex-col gap-4 justify-center" for={@for} as={@as} {@rest}>
